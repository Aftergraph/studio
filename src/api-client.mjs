...
export function createApiClient({ baseUrl='', fetchImpl=globalThis.fetch, EventSourceImpl=globalThis.EventSource }={}) {
  ...
  return Object.freeze({
    ...
    createGoal({owner,successCriteria,budgetCents,horizonEnd,parentGoalId,id,actor='demo-user',idempotencyKey=`goal-${Date.now()}`}={}){return request(apiGoals(),{method:'POST',headers:{'idempotency-key':idempotencyKey},body:JSON.stringify({actor,owner,successCriteria,budgetCents,horizonEnd,parentGoalId,id})})},
    listGoals(){return request(apiGoals())},
    goalProgress(id){return request(apiGoalProgress(id))},
    linkMissionGoal(id,{goalId,actor='demo-user',idempotencyKey=`mission-goal-${id}-${Date.now()}`}={}){return request(apiMissionGoal(id),{method:'PATCH',headers:{'idempotency-key':idempotencyKey},body:JSON.stringify({actor,goalId})})},
    recordLesson({missionId,verdict,lesson,by,goalId,id,actor='demo-user',idempotencyKey=`lesson-${Date.now()}`}={}){return request(apiLessons(),{method:'POST',headers:{'idempotency-key':idempotencyKey},body:JSON.stringify({actor,missionId,verdict,lesson,by,goalId,id})})},
    listLessons({verdict}={}){return request(verdict?`${apiLessons()}?verdict=${encodeURIComponent(verdict)}`:apiLessons())},
