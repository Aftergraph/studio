...
function snapshot(e){return Object.freeze({manifest:e.manifest,state:e.state,freshness:e.freshness,detail:e.detail==null?null:{...e.detail}})}
export function createIntegrationRegistry(){
  ...
  return Object.freeze({
    ...
    setState(id,state,detail=null){if(!STATES.has(state))throw new TypeError(`invalid integration state ${state}`);const e=entries.get(id);if(!e)throw new Error(`unknown integration ${id}`);e.state=state;e.freshness=state==='current'?'current':state;e.detail=detail==null?null:{...detail};return snapshot(e)},
