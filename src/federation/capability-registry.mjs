...
export function createCapabilityRegistry({resolveGrant=()=>false}={}){const caps=new Map();return Object.freeze({
  register(cap){if(!cap?.id||!cap?.source)throw new TypeError('capability id/source required');const frozen=freeze({...cap});if(caps.has(cap.id)){const prior=caps.get(cap.id);if(prior.source!==frozen.source)throw new Error(`capability conflict ${cap.id}`);return prior}caps.set(cap.id,frozen);return frozen},
