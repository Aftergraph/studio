export function renderWorkView({rail='',header='',summary='',attention='',outcome='',trajectory='',composer='',artifact='',artifactOpen=false}={}){
  ...
  return `<main id="main-content" class="ag-workspace-work ag-calm-work ${artifactOpen?'with-artifact':''}">
    ...
    <section class="ag-work-canvas ${artifactOpen?'with-artifact':''}">
      <div class="ag-work-primary"><div class="ag-work-detail ag-outcome-first">${header}${summary}${attention}${outcome}${billingLauncher}<details class="ag-work-trajectory-disclosure"><summary><span>Trajectory</span><small>Inspect execution path</small></summary>${trajectory}</details></div><div class="ag-work-composer">${composer}</div></div>
