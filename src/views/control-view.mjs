...
export function renderControlSurface({localApprovals=[],remoteApprovals=[],exceptions=[],events=[],service={},decisionError=''}={}){
  ...
  return `<main id="main-content" class="ag-domain-page ag-calm-control" data-domain-surface="control">
    ...
    <div class="ag-control-product-grid">
      ...
      <aside class="ag-control-provenance"><section class="ag-domain-section"><div class="ag-section-heading"><span>Authority source</span>${AGSourceTruthBadge({owner:'Trust Gateway',contract:'approval.decide',sha:service.headSha||''})}</div><p class="ag-control-note">Consequential decisions are committed only after their authority owner confirms them.</p></section><section class="ag-domain-section"><div class="ag-section-heading"><span>Recent audit</span><small>${events.length}</small></div>${events.slice(0,8).map(event=>AGEventRow({event})).join('')||'<p class="ag-domain-empty">No recent control events.</p>'}</section></aside>
    </div>
  </main>`;
}
