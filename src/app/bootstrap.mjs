...
export function bootstrapAftergraph(){
  ...
  function renderContextInspector(){
    ...
    if(ui.inspectorKind==='agent'){
      ...
      const memories=state.memory.map(m=>AGMemoryItem({id:m.id,label:m.label,scope:m.scope,source:m.source,promoted:m.promoted})).join('');
