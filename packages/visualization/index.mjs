...
export function AGReplayTimeline({ frames=[], activeIndex=0, playing=false, temporalMode='historical' }={}) {
  const max=Math.max(0,frames.length-1);
  const safe=Math.max(0,Math.min(max,Number(activeIndex)||0));
  const mode=String(temporalMode||'historical');
  const modeControls=`<nav class="ag-temporal-modes" aria-label="Temporal view mode"><button type="button" class="${mode==='historical'?'active':''}" data-temporal-action="historical">Historical</button><button type="button" class="${mode==='counterfactual'?'active':''}" data-temporal-action="counterfactual">Counterfactual</button><button type="button" class="${mode==='forecast'?'active':''}" data-temporal-action="forecast">Forecast</button></nav>`;
  return `<section class="ag-replay-timeline" data-ag-component="replay-timeline" data-temporal-mode="${esc(mode)}"><header><span><small>Time</small><strong>Mission replay</strong></span><span class="ag-replay-controls"><output>${frames[safe]?.event?.time||'now'}</output><button type="button" data-replay-action="${playing?'pause':'play'}" aria-label="${playing?'Pause':'Play'} replay">${playing?'Ⅱ':'▶'}</button></span></header>${modeControls}<input type="range" min="0" max="${max}" value="${safe}" step="1" data-replay-action="scrub" aria-label="Replay timeline"/><div class="ag-replay-events">${frames.map((frame,i)=>`<button type="button" class="${i===safe?'active':''}" data-replay-index="${i}" aria-label="Frame ${i+1}: ${esc(frame.event?.type||'event')}"><i></i><span>${esc(frame.event?.type||'event')}</span></button>`).join('')}</div></section>`;
}
