...
export function parseCompileResponse(value: unknown): CompileResponse {
  ...
  if (typeof ir.schema !== 'string' || !source || typeof source.text !== 'string' || typeof source.surface !== 'string') throw new ComposeProtocolError();
  if (!goal || typeof goal.statement !== 'string' || !isStringArray(goal.successCriteria) || !isStringArray(ir.ambiguities)) throw new ComposeProtocolError();
  if (typeof target.target !== 'string' || typeof target.confidence !== 'number' || !isStringArray(target.reasonCodes) || !isStringArray(target.alternatives)) throw new ComposeProtocolError();
  if (typeof artifact.target !== 'string' || typeof artifact.mediaType !== 'string' || typeof artifact.content !== 'string' || !artifact.semanticMap || typeof artifact.semanticMap !== 'object') throw new ComposeProtocolError();
  return value as CompileResponse;
}

export async function compileIntent(source: string, target: ComposeTarget = 'auto', refinement?: string, signal?: AbortSignal): Promise<CompileResponse> {
